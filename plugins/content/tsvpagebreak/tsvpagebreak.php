 ...
 * 
 * 
 * Kopie von joomla plugin pagebreak.
 * �nderungen:
 * 
 * 1. "Seite 1 von 3"   nach unten verschoben
 * 2. TOC ge�ndert
 */
 ...
 * @since  1.6
 */
class PlgContentTsvpagebreak extends CMSPlugin
{
	/**
	...
	public function onContentPrepare($context, &$row, &$params, $page = 0)
	{
		...
		if ($n > 1)
		{
			...
			if ($style === 'pages')
			{
                ...
                */
				...
				$row->tsv_page_counter = '<div class="pagenavcounter">';
				$row->tsv_page_counter .= $pageNav->getPagesCounter();
				$row->tsv_page_counter .= '</div>';
			}
			else
			{
				$t[] = $text[0];
        ...
        if(isset($row->page_title))  // sonst wird "Fotoalbum" in der Überschrift mit angezeigt
        {
            $row->tsv_page_title = $row->page_title;
            unset($row->page_title);
        }
	}
	...
	protected function _createToc(&$row, &$matches, &$page)
	{
		...
		$this->onlyPhotoalbum = false;
		
		if($i == 3 && $hasPhotoalbum)
		{
		    $this->list[1]->title = 'zurück zum Bericht';
		    if($limitstart == 0)
		      unset($this->list[1]);
		    else if($limitstart == 1)
		      unset($this->list[2]);
		    
		    $this->onlyPhotoalbum = true;
		}
		
		$this->hasPhotoalbum = $hasPhotoalbum;
		...
		$list = $this->list;
		$path = PluginHelper::getLayoutPath('content', 'tsvpagebreak', 'toc');
		ob_start();
		include $path;
	...
	protected function _createNavigation(&$row, $page, $n)
	{
		...
		$path = PluginHelper::getLayoutPath('content', 'tsvpagebreak', 'navigation');
	
		ob_start();
		include $path;
		//$row->text .= ob_get_clean();
		$row->tsv_navigation = ob_get_clean();
	}
}
